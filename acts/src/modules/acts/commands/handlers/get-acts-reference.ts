...
import { GetActsOfCustomerCommand, GetActsOfGCustomerCommand, GetActsOfLabCommand } from '../impl/get-acts-reference';
...
export class GetActsOfGCustomerHandler implements ICommandHandler<GetActsOfGCustomerCommand> {
    logger = new Logger(this.constructor.name)
    ...
    async execute(command: GetActsOfGCustomerCommand): Promise<Act[]> {
        ...
        try {return (await this.gcusrtomerRepository.findOne(gcustomerId)).acts} catch(e) {
            this.logger.error(e)
        }
    }
}
...
export class GetActsOfLabHandler implements ICommandHandler<GetActsOfLabCommand> {
    logger = new Logger(this.constructor.name)
