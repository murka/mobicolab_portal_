...
export class Init1599056353060 implements MigrationInterface {
    name = 'Init1599056353060'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TYPE "act_status_enum" AS ENUM('CREATED', 'GENETATED', 'REGISTERED', 'REMARKED', 'CORRECTED', 'FULL')`);
        await queryRunner.query(`CREATE TABLE "act" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "name" character varying NOT NULL, "objectName" character varying, "place" character varying, "method" character varying, "toolType" character varying, "climaticEnvironmental" character varying, "planning" character varying, "normativeDocument" text array, "sampleType" character varying, "sample" text array, "preparation" text array, "goal" character varying, "definedIndicators" text array, "additions" character varying, "informationAboutSelection" character varying, "environmentalEngineer" character varying, "representative" character varying, "passedSample" character varying, "status" "act_status_enum" NOT NULL DEFAULT 'CREATED', "customerId" character varying, "generalCustomerId" character varying, "labId" character varying, "typeOfSampleId" uuid, "datetimeDate" TIMESTAMP WITH TIME ZONE DEFAULT now(), "datetimeTime" character varying, CONSTRAINT "PK_84c5378dc7f3b8355b6e8c3ccb0" PRIMARY KEY ("id"))`);
