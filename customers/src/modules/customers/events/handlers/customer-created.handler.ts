...
export class CustomerCretedHandler implements IEventHandler<CustomerCreatedEvent> {
  ...
  async handle(event: CustomerCreatedEvent) {
    ...
    try {
      const newEvent = this.eventRepository.create({customer: customer, event: 'CREATED' });
